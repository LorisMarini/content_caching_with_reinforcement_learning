...
N_Ini = 10;       % Number of routines for initialisation of estimates D
...
Learned_Action = S( New_P(i-1,:) > 0.999);

if Learned_Action == Opt_Act
    
   disp('The algorithm pursued the CORRECT action. ');
   disp(['Total number of iterations ' num2str(i)] );
else
    disp('The algorithm pursued the wrong action.');
end
