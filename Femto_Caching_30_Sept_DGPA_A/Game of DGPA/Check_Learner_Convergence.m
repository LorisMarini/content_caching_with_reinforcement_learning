function [ Learner_Convergence New_P_Vector Action] = Check_Learner_Convergence( Probability_Vector, P_Threshold )
...
Converged_Actions = sum(Convergence_Index);
    ...
    elseif Converged_Actions == 1
           ...
           [Value, Action] = find(Convergence_Index);
           if sum(New_P_Vector) ~= 1
               error('The Function Check_Learner_Convergence is ill designed.');
           end           
    elseif Converged_Actions > 1
           Learner_Convergence = true;
           [Value Action] = max(Probability_Vector(Convergence_Index));
