function [ OUT_Learning, OUT_Converged_Actions ] = New_P_Vectors_DGPA_A(  Conv_Actions, IN_Learning, Delta, P_Threshold )
...
for j = 1:1:H
    for k = 1:1:M
        ...
        elseif (Conv_Actions(k,j) == 0) % Learner (k,j) has not converged yet. Let's update its P & D.
            ...
            for o = 1:1:r
                if (o ~= A)
                    if ( Curr_D(o) <= Curr_D(A) )
                        Direction_Vector(o) = 0;
                    elseif ( Curr_D(o) > Curr_D(A))
                        Direction_Vector(o) = 1;
                    end
                elseif (o == A)
                    [Value, Position] = max( Curr_D );
                    Number_of_Maxima = sum ( Curr_D == Value );
                    ...
                    elseif Number_of_Maxima > 1  % Multiple Maxima
                        Maxima =  Curr_D == Value;
                        if sum ( Curr_D(Maxima) == Curr_D(A) ) > 1
                            Direction_Vector(o) = 1;
                        else
                            Direction_Vector(o) = 0;
                        end
