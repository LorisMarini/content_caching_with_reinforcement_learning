...
N_Ini = 20;         % Initial #Iterations for estimation of D from each Learner 
...
while (Lesser_Selected < N_Ini || Zeros > 0)
    ...
    for j = 1:1:H
        for k = 1:1:M    
            ...
            if k > 1 
                while sum(Action == Available_Files(1:k-1,j))
                    Action = randsrc(1,1,[ S; Learning(k,j).P ]);
                end
                Available_Files(k,j) = Action;
            end
    ...
    for n = 1:1:N
      ...
      %% USER's SELECTION (NCA)
        ...
        for f = 1:1:F         
            ...
            % NCA (Nearest Content Available) policy:
            ...
            for j = 1:1:H+1
                for k = 1:1:M
                    ...
                    elseif ( j ~= S_ID_Selected)
                        ...
                        elseif(Alternatives(k,j) == 0)
                            % Set to Infinity all others (either because
                            % file 'f' is not abailable or because there is
                            % no connectivity.
                            User_Selections(k,j,f) = Inf;
                        end
                    end      
      ...
      %% USER's AVERAGE DELAY
        ...
        if (N_Files_Selected ~= F)
            error('ERROR I_234: Number of selections does not mach the number of files F.');
        end
        %}
      ...
      %% USER's FEEDBACK TO THE SOURCES  
        ...
        for f = 1:1:F  % For all F files (f)
            for j = 1:1:H+1 % For all Sources (j)
                for k = 1:1:M % For all Lerners (k)   
                    ...
                    elseif (Weighted_Delay > Minimum_Weighted_Delay(n) )
                        if (User_Selections(k,j,f) >= 0 && User_Selections(k,j,f) ~= Inf)
                            Penalties(k,j) = Penalties(k,j) + Popularities(f);
                            N_Penalties(k,j) = N_Penalties(k,j) +1;
                        elseif (User_Selections(k,j,f) == Inf)
        ...
        if ( Feedbacks_Received(:,1:end-1) > 1 )
            error('There is a problem with the users decision');
        end
        
        % Update the minimum delay for the user at each iteration.
        if ( Weighted_Delay < Minimum_Weighted_Delay(n) )
            Minimum_Weighted_Delay(n) = Weighted_Delay;     
        elseif ( Weighted_Delay >= Minimum_Weighted_Delay(n) )
            Minimum_Weighted_Delay(n) = Minimum_Weighted_Delay(n);
        end
     ...
     %% Learners Determine the Environment Feedback Democratically
      ...
      for j = 1:1:H
        for k = 1:1:M          
            ...
            elseif(Rewards(k,j) < Penalties(k,j))
      ...
      disp(['Iteration ' num2str(ITER) ' The min weighted delais are: ' num2str(Minimum_Weighted_Delay) '.There are ' num2str(Zeros) ' learners with a zero D vector.']);
...
GITER = 1;                                    % Stability Initialisation.
