function [ OUT_Learning, OUT_Converged_Actions ] = Move_P_Vector( Conv_Actions, IN_Learning, Delta, P_Threshold )
...
for j = 1:1:H
    for k = 1:1:M
        ...
        elseif (Conv_Actions(k,j) == 0) % Learner (k,j) has not converged yet. Let's update its P & D.
            ...
            for o = 1:1:r
                if ( o ~= A_Pos )
                    if ( Curr_D(o) <= Curr_D(A_Pos)  &&  IN_Learning(k,j).P( o )~= 0)
                        Direction_Vector(o) = 0;
                    elseif ( Curr_D(o) > Curr_D(A_Pos) &&  IN_Learning(k,j).P( o )~= 0)
                ...
                elseif (o == A_Pos)
                    ...
                    elseif Number_of_Maxima > 1  % Multiple Maxima
                        ...
                        if sum ( Curr_D(Index) == Curr_D(A_Pos) ) > 1
                            Direction_Vector(o) = 1;
                        else
                            Direction_Vector(o) = 0;
                        end
            ...
            % 4) Check Convergence
            if ( Check_Learner_Convergence( Space, IN_Learning(k,j).P, P_Threshold ) )  % The Larner (k,j) has converged to its final decision,therefore, we can update its P vector:
                [ LC, New_Prob_Vector, Action] = Check_Learner_Convergence( Space,IN_Learning(k,j).P, P_Threshold );
